;(function () {
  ...
  var Game = function () {
    ...
    // This contains functions that let you draw to the canvas.
    var screen = canvas.getContext('2d')

    // Note down the dimensions of the canvas.
    // These are used to place game bodies.
    var gameSize = {
      x: canvas.width,
      y: canvas.height
    }
    ...
    // Run the first game tick.
    // All future calls will be scheduled by the tick() function itself.
    tick()
  }
  ...
  Game.prototype = {
    ...
    update: function () {
      ...
      // Throw away bodies that are colliding with something.
      // They will never be updated or draw again.
  ...
  var Invader = function (game, center) {
    ...
    // `this.patrolX` records the current (relative) position of the invader in their patrol.
    // It starts at 0, increases to 40, then decreases to 0, and so forth.
    this.patrolX = 0

    // The x speed of the invader.
    // A positive value moves the invader right, A negative value moves it left.
    this.speedX = 0.3
  }
